...
function GetSteamAppsList() {
  ...
  if (!steamPath) {
    return [
      { title: '无法找到Steam', description: 'Steam或许未被正常安装', url: '' },
    ]
  } else {
    let appList = []
    ...
    let content = fs
      .readFileSync(path.join(steamAppsPathList[0], 'libraryfolders.vdf'))
      .toString()
    try {
    ...
    for (let i in steamAppsPathList) {
      ...
      try {
        ...
        for (let j in fileList) {
          ...
          if (extname == '.acf') {
            let content = fs
              .readFileSync(path.join(steamAppsPath, file))
              .toString()
            try {
    ...
    for (let i in charList) {
      ...
      if (nowIndex == -1) {
        ...
        if (nowIndex != 0) {
          ...
          if (alphaRegex.test(prevChar)) matchScore += nowIndex - lastIndex
        }
        lastIndex = nowIndex
      }
    }
